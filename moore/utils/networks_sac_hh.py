...
import moore.utils.mixture_layers_hh as mixture_layers
...
class MetaworldSACMixtureMHCriticNetworkHH(nn.Module):
    def __init__(self, input_shape,
        ...
        if orthogonal:
            self._h = nn.Sequential(mixture_layers.InputLayer(n_models=n_experts),
                                    ...
                                    mixture_layers.OrthogonalLayerHouseholder1D(),
                                    )
        else:
            self._h = nn.Sequential(mixture_layers.InputLayer(n_models=n_experts),
                                    mixture_layers.ParallelLayer(self._h),
    ...
    def forward(self, state, action=None, c=None):
        ...
        for _, ci in enumerate(torch.unique(c)):
...
class MetaworldSACMixtureMHActorNetworkHH(nn.Module):
    def __init__(self, input_shape,
                       output_shape,
                       n_features,
                       activation='ReLU',
